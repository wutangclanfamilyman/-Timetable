...
<html lang="ru-RU">
  <head>
    ...
    <href type="text/javascript" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBe-P7g_p1JjWk6k3qHHqMIkF4fkUv7MxM&libraries=places">
    </href>
    ...
    <script type="text/javascript" src="js/map_script.js">
    </script>
  ...
  <body onload="showCity()">
    ...
    <div id="RoutePanel">
      <ul class="nav nav-tabs">
        <li class="active">
          <a data-toggle="tab" href="#route">Побудова маршруту
          </a>
        </li>
        <li>
          <a data-toggle="tab" href="#timetable">Розклад руху
          </a>
        </li>
      </ul>
            ...
            <div id="formSearch">
              <div class="row input-start">
                <div class="form-group col-sm-12">
                  ...
                  <button onclick="showStopInputA()" title="Показати на карті">
              ...
              <div class="row input-finish">
                <div class="form-group col-sm-12">
                  ...
                  <button onclick="showStopInputB()" title="Показати на карті">
                    <i class="fa fa-map-marker-alt">
                    </i>
                  </button>
                </div>
              ...
              <div class="row btn-send texc-center">
                <div class="form-group col-sm-offset-1 col-sm-10 col-sm-offset-1">
                  <a class="btn" onclick="getRouteNotTransfer()" id="btn-search">Пошук
                  </a>
                </div>
              </div>
              <datalist id="stops-for-inputs">
              </datalist>
            </div> 
            <hr>
          </div>
          ...
          <div class="row">
            <div class="direction text-center">
              <label class="name">Напрям маршруту 
                ...
                <button id="saveRouteUser" value='' onclick="saveRouteUser(<?php echo $_SESSION['user_id'] ?>, value)" title="Зберегти маршрут в профіль">
                  <i class="fa fa-save">
                  </i>
                </button>
                <?php endif; ?>
              ...
              <select id="direction" onchange="ShowTimeDeparture(this.value); getPolylineForRoute(routeNum,this.value);">
                <option class="active" value="">Не обрано
                </option>
              </select>
            </div>
          </div>
      ...
      <div class="tab-content" id="tabsMenu">
        <div id="tab-user-numbers" class="tab-pane fade in active">
          ...
          <label class="label label-success">
            <a data-toggle="tab" id="getUserRoute" onclick="getRouteByUser(<?php echo $_SESSION['user_id'] ?>);" href="#tab-user-numbers">Мої маршртури
            </a>
          </label>
          <form method="post" action="ajax.php">
            <input type="hidden" name="act" value="logout">
            <label class="label label-danger">
              <button type="submit">Вихід
              </button>
            </label>
          </form>
        </div>
      ...
      <div class="tab-content" id="tabsMenu">
        <div id="authorization" class="tab-pane fade in active">
          <form method="post" action="ajax.php">
            ...
            <div class="form-group text-center">
              <button type="submit" class="btn btn-primary">Авторизація
              </button>
            </div>
            <input type="hidden" name="act" value="login">
            <div class="form-group register">
              <label class="label label-success">
                <a data-toggle="tab" href="#registration">Реєстрація
                </a>
              </label>
            </div>
          </form>
        ...
        <div id="registration" class="tab-pane fade">
          <form method="post" action="ajax.php">
            ...
            <div class="form-group">
              <label for="exampleInputPassword1">Повторіть пароль
              </label>
            ...
            <div class="form-group text-center">
              <button id="register-user-btn" onclick="registerUser()" class="btn btn-primary">Реєстрація
              </button>
            </div>
            <input type="hidden" name="act" value="register">
            <div class="form-group authorization">
              <label class="label label-success">
                <a data-toggle="tab" href="#authorization">Авторизація
                </a>
              </label>
            </div>
          </form>
    ...
    <div id="user-button">
      <?php if (Auth\User::isAuthorized()): ?>
      <button class="btn" id="btn-form" title="Поле користувача">
        <i class="fa fa-user">
        </i>
      </button>
      <?php else: ?>
      <button class="btn" id="btn-form" title="Авторизація">
        <i class="fa fa-sign-in-alt">
        </i>
      </button>
      <?php endif; ?>
    </div>
    <div id="my-location-button">
      <button class="btn" id="btn-my-location" onclick="getLocation()" title="Моє місцеположення">
